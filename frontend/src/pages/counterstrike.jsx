...
export default function CS2() {
  ...
  useEffect(() => {
    ...
    const profileRecentMatches = data?.recent_matches
...
const mapStats = matchesData ? Object.values(
  ...
  console.log("match outcomes:", matchesData?.slice(0, 3).map(m => m.outcome))
  console.log("first match keys:", matchesData?.[0] ? Object.keys(matchesData[0]) : null)
  ...
  return (
    <div className={styles.container}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "2rem" }}>
        <button onClick={() => setChartView("recent")} className={chartView === "recent" ? styles.activeButton : styles.button}>
          Recent Matches
        </button>
        <button onClick={() => setChartView("map")} className={chartView === "map" ? styles.activeButton : styles.button}>
          Per Map
        </button>
      </div>
